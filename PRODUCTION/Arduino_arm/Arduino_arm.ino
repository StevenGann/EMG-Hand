...
void loop() 
{
	while(Serial1.available() > 0) //when there's information being received through serial go into this loop
	{
		...
		shoulderTarget = Serial1.parseInt();
		elbowTarget = Serial1.parseInt();
		servoVal[0] = Serial1.parseInt();
		servoVal[1] = Serial1.parseInt();
		servoVal[2] = Serial1.parseInt();
		servoVal[3] = Serial1.parseInt();
		servoVal[4] = Serial1.parseInt();
		
		int restOfPacket[3];
		
		restOfPacket[0] = Serial1.parseInt();
		restOfPacket[1] = Serial1.parseInt();
		restOfPacket[2] = Serial1.parseInt();
